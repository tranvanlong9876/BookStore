...
public class UpdateBookServlet extends HttpServlet {
    ...
    private static final String UNSUCCESS = "LoadBookToUpdateServlet";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
                ...
                if (account.getRolename().equals("Admin")) {
                    ...
                    if (checkValid.isIsValidBook()) {
                        if (checkBoxNewCategory != null) {
                            if (ValidateInput.validateNewCategory(txtNewCategory)) {
                                if (!dao.checkExistedCategories(txtNewCategory)) {
                                    ...
                                    if (newBookCategoryID > 0) {
                                        ...
                                        if (dao.updateSelectedBook(newBook, bookID)) {
                                            url = SUCCESS;
                                            request.setAttribute("SEARCHBOOK_STATUS", "Thông tin mã sách: " + bookID + " kèm danh mục mới đã được cập nhật thành công.");
                                        }
                                    }
                                } else {
                                    url = UNSUCCESS;
                            ...
                            if (dao.updateSelectedBook(newBook, bookID)) {
                                url = SUCCESS;
                                request.setAttribute("SEARCHBOOK_STATUS", "Thông tin mã sách: " + bookID + " đã được cập nhật thành công.");
                            }
                        }
                    } else {
                        url = UNSUCCESS;
            ...
            String errorServlet = "Lỗi đã xảy ra khi cập nhật thông tin của sách";
